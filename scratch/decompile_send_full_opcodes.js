...
function parseSendDetailed(codeOff) {
  ...
  while (i < insnsSize) {
    ...
    if (op === 0x12) {
      ...
      desc = `const/4 v${vA}, #${vB}`;
    } else if (op === 0x07) {
      const vA = dex[offset + 1];
      const vB = readUShort(offset + 2);
      desc = `move-object v${vA}, v${vB}`;
      len = 2;
    } else if (op === 0x08) {
      const vA = dex[offset + 1];
      const vB = readUShort(offset + 2);
      desc = `move-object/from16 v${vA}, v${vB}`;
      len = 2;
    } else if (op === 0x10) {
      const vA = readUShort(offset + 2);
      const vB = readUShort(offset + 4);
      desc = `move-object/16 v${vA}, v${vB}`;
      len = 3;
    } else if (op === 0x11) {
      const vA = readUShort(offset + 2);
      const vB = readInt(offset + 4);
      desc = `move-object/16 v${vA}, v${vB}`; // wait, let's be careful
      len = 3;
    } else if (op === 0x22) {
      const reg = dex[offset + 1];
      const typeIdx = readUShort(offset + 2);
      desc = `new-instance v${reg}, ${types[typeIdx]}`;
      len = 2;
    } else if ((op >= 0x6E && op <= 0x72) || (op >= 0x74 && op <= 0x78)) {
      const methodIdx = readUShort(offset + 2);
      const target = methods[methodIdx];
      const name = op >= 0x74 ? 'invoke/range' : 'invoke';
      desc = `${name} method#${methodIdx} (${target ? target.class + ' -> ' + target.name : 'unknown'})`;
      len = 3;
    } else if (op === 0x0c) {
      const reg = dex[offset + 1];
      desc = `move-result-object v${reg}`;
    } else if (op === 0x0e) {
      desc = `return-void`;
    }
    
    console.log(`  +${i}: [0x${op.toString(16).toUpperCase()}] ${desc || ('unknown opcode 0x' + op.toString(16))}`);
    i += len;
  }
}

parseSendDetailed(17164);
