...
// Find class def for BaseActivity
let baseActivityDefOff = 0;
for (let i = 0; i < classDefsSize; i++) {
  const classDefsOffIdx = classDefsOff + i * 32;
  const classIdx = readInt(classDefsOffIdx);
  if (types[classIdx] === 'Lcom/tools/pipra_pay/Class/BaseActivity;') {
    baseActivityDefOff = classDefsOffIdx;
    break;
  }
}

console.log('BaseActivity class_def offset:', baseActivityDefOff);

// Read class_data_item
const classDataOff = readInt(baseActivityDefOff + 24);
