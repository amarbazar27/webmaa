...
for (let i = 0; i < classDefsSize; i++) {
  ...
  if (className.startsWith('Lcom/tools/pipra_pay/')) {
    log(`=========================================`);
    log(`Class: ${className}`);
    ...
    for (let f = 0; f < staticFieldsSizeInfo.value; f++) {
      offset += readUleb128(dex, offset).bytesRead;
      offset += readUleb128(dex, offset).bytesRead;
    }
    ...
    for (let m = 0; m < directMethodsSizeInfo.value; m++) {
      ...
      readMethodsList.push({ idx: methodIdx, codeOff: codeOff.value });
    }
    ...
    readMethodsList.forEach(m => {
      ...
      log(`Method: ${methodInfo.name} (codeOff: ${m.codeOff})`);
...
fs.writeFileSync('d:/webmaa/scratch/all_classes_decompile.txt', out, 'utf8');
console.log('Saved to all_classes_decompile.txt');
