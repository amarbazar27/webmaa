...
function decompileMethod(codeOff, methodName) {
  ...
  while (i < insnsSize) {
    ...
    // We only care about method calls and string loads to see the URL/endpoints
    if (op >= 0x6E && op <= 0x72) {
      // invoke
      const methodIdx = readUShort(offset + 2);
      const target = methods[methodIdx];
      if (target) {
        console.log(`  +${i}: invoke -> ${target.class} -> ${target.name}`);
      }
    } else if (op === 0x1A || op === 0x1B) {
      // const-string
      const stringIdx = op === 0x1A ? readUShort(offset + 2) : readInt(offset + 2);
      console.log(`  +${i}: const-string -> "${strings[stringIdx]}"`);
    } else if (op === 0x62) {
      // sget-object
      const fieldIdx = readUShort(offset + 2);
      console.log(`  +${i}: sget-object`);
    }
...
decompileMethod(18160, 'lambda$doWork$0');
decompileMethod(17164, 'MainActivity.send');
