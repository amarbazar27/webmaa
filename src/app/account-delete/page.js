...
export default function AccountDeletePage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Attempt auth client deletion or logout
      try {
        await user.delete();
      } catch (authErr) {
        console.warn('Client auth user deletion skipped (expected if session is old):', authErr.message);
      }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast.error('লগইন ব্যর্থ হয়েছে: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between py-12 px-6 relative overflow-hidden font-sans">
      ...
      <div className="max-w-2xl w-full mx-auto bg-white border border-slate-200/60 p-8 md:p-12 rounded-[2.5rem] shadow-2xl relative z-10">
        <Link href="/" className="inline-flex items-center gap-2 text-xs font-black text-slate-500 hover:text-purple-600 transition-colors uppercase tracking-wider mb-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Delete Your Account</h1>
          ...
          <div className="space-y-8">
            ...
            <div className="border-t border-slate-100 pt-6 space-y-4">
              ...
              <ul className="text-xs text-slate-600 font-bold bg-slate-50 border border-slate-100 p-6 rounded-2xl space-y-2.5 list-disc pl-6 leading-relaxed">
                <li>Email us at: <span className="font-mono text-purple-600">support@bdretailers.com</span> or <span className="font-mono text-purple-600">missionedu306@gmail.com</span></li>
        ...
        <div className="border-t border-slate-100 pt-6 mt-8">
          <h4 className="font-extrabold text-slate-900 text-xs uppercase tracking-widest mb-3">আইনগতভাবে সংরক্ষিত ডেটা (Data Retention Policy)</h4>
          <p className="text-[11px] text-slate-400 font-bold leading-relaxed">
            The following data may be retained where required by law or for security: completed order invoices, tax records, and fraud prevention logs.
          </p>
        </div>
      </div>
