...
export default function CallbackUI({ shopSlug, orderId, shopId, status, txnId }) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md overflow-hidden shadow-xl flex flex-col">
        ...
        <div className={`p-8 text-center space-y-4 flex flex-col items-center ${
          ...
          <div>
            <h1 className={`text-xl font-black ${isSuccess ? 'text-emerald-950' : isCancelled ? 'text-amber-950' : 'text-red-950'}`}>
              {isSuccess ? 'পেমেন্ট সফল হয়েছে!' : isCancelled ? 'পেমেন্ট বাতিল করা হয়েছে' : 'পেমেন্ট ব্যর্থ হয়েছে'}
            </h1>
            <p className={`text-xs font-bold mt-1 ${isSuccess ? 'text-emerald-700/80' : isCancelled ? 'text-amber-700/80' : 'text-red-700/80'}`}>
              {isSuccess
          ...
          <div className="space-y-3">
            {isSuccess ? (
              <button
                ...
                className="w-full py-4 bg-slate-900 hover:bg-purple-600 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 transition-colors shadow-lg"
              >
