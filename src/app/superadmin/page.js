  ...
  useEffect(() => { 
    ...
    const unsubscribe = subscribeGlobalConfig((configData) => {
      setGlobalConfig({
        ...
        trialsEnabled: configData?.trialsEnabled ?? true,
        subTrialMonthly: configData?.subTrialMonthly || 30,
    ...
    <div className="space-y-8 animate-slide-in pb-12">
      ...
      <Card title="Platform Intelligence" subtitle="Manage global AI nodes and API keys (Groq Engine)" icon={Sparkles} className="border-2 border-purple-100 bg-purple-50/20">
        <form onSubmit={handleUpdateConfig} className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            ...
            {/* ── Platform Authentication Settings (প্ল্যাটফর্ম লগইন সেটিংস) ── */}
            <div className="md:col-span-12 grid grid-cols-1 gap-4 pt-6 border-t border-purple-100 animate-slide-in">
              <div>
                <p className="text-xs font-black text-slate-900 mb-1 flex items-center gap-2"><ShieldAlert size={14}/> Platform Authentication Settings (প্ল্যাটফর্ম লগইন সেটিংস)</p>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-4">প্রধান ওয়েবসাইটে ক্রেতা/রিটেইলারদের লগইন করার অপশনসমূহ চালু বা বন্ধ করুন</p>
              </div>
              <div className="flex flex-col gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100 max-w-2xl">
                <div className="flex items-center justify-between">
                   <div>
                      <p className="text-xs font-black text-slate-900">Email & Password Auth (ইমেইল ও পাসওয়ার্ড লগইন)</p>
                      <p className="text-[9px] text-slate-500 font-bold uppercase mt-1">Allow registration & login using email and password</p>
                   </div>
                   <label className="relative inline-flex items-center cursor-pointer">
                     <input type="checkbox" className="sr-only peer" checked={globalConfig.emailPasswordAuth !== false} onChange={async (e) => {
                       const updated = {...globalConfig, emailPasswordAuth: e.target.checked};
                       setGlobalConfig(updated);
                       await updateGlobalConfig(updated).catch(() => {});
                     }} />
